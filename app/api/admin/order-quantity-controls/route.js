  ...
  if (
    ...
    return {
      error: NextResponse.json(
        { success: false, error: "Only admin or manager can configure order quantity controls." },
        { status: 403 },
      ),
    };
  }
...
async function readControls(admin) {
  ...
  return {
    controls: configured
      ...
      : normalizeOrderQuantityControls(DEFAULT_ORDER_QUANTITY_CONTROLS),
    configured,
...
export async function GET(request) {
  try {
    ...
    const payload = await readControls(admin);
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Unable to load order quantity controls." },
      { status: 400 },
    );
  }
}
...
export async function PUT(request) {
  try {
    ...
    const controls = normalizeOrderQuantityControls(body.controls);
    const nowIso = new Date().toISOString();

    const { error } = await admin.from("price_catalog_cache").upsert(
      {
        cache_key: ORDER_QUANTITY_CONTROLS_CACHE_KEY,
        price_map: { controls },
        sheet_items: [],
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Unable to save order quantity controls." },
      { status: 400 },
    );
  }
}
