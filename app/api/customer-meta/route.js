...
async function resolveScope(admin, token) {
  ...
  const visibleProfiles = (profilesRes.data || []).filter((profile) => {
  ...
  return {
    ...
    visibleSalesmanCodes: [...new Set(visibleProfiles.map((profile) => normalizeCode(profile.salesman_code)).filter(Boolean))],
  };
}
