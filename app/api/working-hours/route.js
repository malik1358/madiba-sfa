...
export async function GET(request) {
  try {
    ...
    if (!viewAll && userId && userId !== user.id) {
      return Response.json(
        { success: false, error: "Not authorized to view other users" },
        { status: 403 },
      );
    }

    // Salesmen are not granted the module; keep API aligned with moduleAccess.
    if (!viewAll) {
      return Response.json(
    ...
    const report = await buildDailyWorkingHoursReport(admin, {
      date,
      userIdFilter,
    });
    ...
    return Response.json(
      { success: false, error: error.message || "Unable to load working hours" },
      { status: 400 },
    );
  }
}
