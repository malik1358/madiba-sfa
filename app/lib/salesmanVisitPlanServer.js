...
import {
  ...
  SALESMAN_VISIT_PLAN_REBUILD_STATUS_KEY,
...
function rememberLatestVisit(map, customerCode, visitAt) {
  const code = normalizeCode(customerCode);
  const at = String(visitAt || "").trim();
  if (!code || !at) return;
...
export async function readVisitPlanRebuildStatus(admin) {
  const { data, error } = await admin
    ...
    .eq("setting_key", SALESMAN_VISIT_PLAN_REBUILD_STATUS_KEY)
    .maybeSingle();
...
export async function writeVisitPlanRebuildStatus(admin, status) {
  const payload = {
  ...
  const { error } = await admin.from("system_settings").upsert({
    setting_key: SALESMAN_VISIT_PLAN_REBUILD_STATUS_KEY,
...
export function mergeVisitPlanCustomerCandidates(
  ...
  (collectionRecords || []).forEach((record) => {
    ...
    const merged = {
      ...
      outstanding_0_30: Number(due?.outstanding_0_30 ?? record.outstanding_0_30 ?? existing.outstanding_0_30 ?? 0),
      outstanding_30_60: Number(due?.outstanding_30_60 ?? record.outstanding_30_60 ?? existing.outstanding_30_60 ?? 0),
      outstanding_61_90: Number(due?.outstanding_61_90 ?? record.outstanding_61_90 ?? existing.outstanding_61_90 ?? 0),
      outstanding_91_120: Number(due?.outstanding_91_120 ?? record.outstanding_91_120 ?? existing.outstanding_91_120 ?? 0),
      outstanding_above_120: Number(due?.outstanding_above_120 ?? record.outstanding_above_120 ?? existing.outstanding_above_120 ?? 0),
...
export async function loadSalesmanVisitPlanFromSnapshot(admin, {
  ...
  const [snapshot, rebuildStatus] = await Promise.all([
    readSalesmanVisitPlanSnapshot(admin),
    readVisitPlanRebuildStatus(admin).catch(() => null),
  ]);
  return {
    ...filterVisitPlanSnapshot(snapshot, { salesmanCode, limit }),
    rebuildStatus,
  };
}
