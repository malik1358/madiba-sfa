...
- Day-route / daily visit **Working hours** (`resolveDayRouteWorkingHours` in `app/lib/dayRouteMap.js`): prefer the span from the first to the last **non-far** customer transaction (`VISIT_REPORT`, `COLLECTION_VISIT`, `ORDER_SUBMITTED`) at or after **08:00 KSA**, minus lunch overlap. Pre-8:00 KSA stops (including midnight activity) and far-from-customer rows do not set those endpoints. When that span cannot be computed (no two qualifying stops), fall back to attendance hours: login→lunch out + lunch in→logout, with login clamped to not start before 08:00 KSA. The Day route UI and visit-report email always show the Working hours line (value may be `-`). This is separate from the 06:00–22:00 helper window and from the User Activity report’s own working-hours column.
...
- Pending Orders shows both `Current outstanding` and `Outstanding >60 days` from the uploaded outstanding dataset for the customer. The >60 value is the sum of buckets `61-90`, `91-120`, and `>120`, and the queue de-dupes by normalized customer identity so same-customer rows are not double-counted.
