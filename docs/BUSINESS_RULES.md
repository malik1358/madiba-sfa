...
- Outstanding Compare “differences only” report (customer level): Tally = customer total on the outstanding upload (bucket sum); SFA = `outstandingCompareTotals.computed_open`. A customer is listed when |SFA − Tally| > 0.02 or any invoice has an open gap. Customer name opens Payment Settlement → Invoices & Settlement.
