...
- Receipt copies and payment copies go to the `payment-collections` bucket. Text-only collection visits save on-device first (`queueFirst`) and sync in the background so flaky mobile data cannot block collectors. Attachment (Funds Received) saves still try the server first (no IndexedDB serialize-first for large PDFs/photos), then queue on timeout — sync re-resolves Android MIME. Photo compression is time-bounded with a fallback to the original file, bucket MIME refresh is best-effort, and Saving clears before the queue reload. Save enrichment uses the queue row’s `avg_days_to_pay` and skips the client activity-timeline fetch; the API patches previous-visit distance when the visit syncs.
