...
- **2026-09-25** — Outstanding Compare now has a “Tally vs SFA — differences only” report. It scans the visible (search-filtered) customers client-side, 4 at a time, and keeps only customers where SFA computed open (cash FIFO + CN) differs from the Tally customer total by more than 0.02 or any invoice has an open gap (`buildCustomerOutstandingReconcileRow`). Customer name links to Payment Settlement `#invoices-settlement` (that page now scrolls to the hash after load). `/api/customer-history` accepts `lite=1`: no peer rows and no cache write, for bulk scans. No migration.
