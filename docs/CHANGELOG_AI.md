...
- **2026-09-26** — Day-route / Working Hours report: no non-far customer stops → **0h** (removed login→logout attendance fallback that inflated idle-only days like login + GPS + auto-logout to ~15h).
- **2026-09-26** — Payment Collections WhatsApp/queue **Salesman** for customer **1224** showed ABDUL REHMAN after the 24-Sep outstanding upload left every Salesman cell blank and the queue fell back to a stale customer-master assignment. Fix: when upload Salesman is blank, enrich invoices from `active_sales` by voucher/`ref_no`, then `resolveCollectionQueueSalesman` (prefer master when it matches an open invoice, else voucher, else master). Stored visit summaries no longer substitute `scheduled_by_name` (collector) for Salesman. Production data: 1224 master set to AHMED NABIL (previous ABADALLA ANTHANATH); today's visit summaries patched.
