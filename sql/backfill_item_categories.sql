...
where im.item_code = ranked.item_code
  and (
    ...
    im.category is null or trim(im.category) = '' or upper(trim(im.category)) = 'UNCLASSIFIED'
  );
...
where sr.item_code = im.item_code
  and (
    ...
    sr.category is null or trim(sr.category) = '' or upper(trim(sr.category)) = 'UNCLASSIFIED'
  );
