...
test("pending time runs from order created until now when invoice is not made", () => {
...
test("pending time freezes after invoice upload", () => {
  ...
  assert.equal(isPendingOrderTimeFrozen(meta), true);
  assert.equal(pendingOrderTimeToMakeSeconds(order, meta, Date.parse("2026-09-10T12:00:00.000Z")), 3630);
});
